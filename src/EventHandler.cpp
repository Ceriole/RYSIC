	...
	Action *GameplayEventHandler::get_action_from_SDL_event(SDL_Event &event)
	{
		...
		if(event.type == SDL_KEYDOWN)
		{
			...
			case SDLK_v:
				if(!m_shift) break;
			case SDLK_SEMICOLON:
				return new StartTargetAction(m_game, m_world, m_world->player()->pos);
				break;
			case SDLK_r:
			...
			m_world->message(fmt::format(Constants::MSG_NO_ACTION_FOR_KEY, SDL_GetKeyName(event.key.keysym.sym)), Log::MessageType::SYSTEM);
		}
		return nullptr;
	}
	
	Action *TargetEventHandler::get_action_from_SDL_event(SDL_Event &event)
	{
		if(event.type == SDL_KEYDOWN)
		{
			switch(event.key.keysym.sym)
			...
			case SDLK_KP_8:
			case SDLK_j:
				return new MoveTargetAction(m_world, {0, -1}); // up
			case SDLK_DOWN:
			case SDLK_KP_2:
			case SDLK_k:
				return new MoveTargetAction(m_world, {0, 1}); // down
			case SDLK_LEFT:
			case SDLK_KP_4:
			case SDLK_h:
				return new MoveTargetAction(m_world, {-1, 0}); // left
			case SDLK_RIGHT:
			case SDLK_KP_6:
			case SDLK_l:
				return new MoveTargetAction(m_world, {1, 0}); // right
			case SDLK_KP_7:
				return new MoveTargetAction(m_world, {-1, -1}); // up-left
			case SDLK_KP_9:
				return new MoveTargetAction(m_world, {1, -1}); // up-right
			case SDLK_KP_1:
				return new MoveTargetAction(m_world, {-1, 1}); // down-left
			case SDLK_KP_3:
				return new MoveTargetAction(m_world, {1, 1}); // down-right
			case SDLK_KP_ENTER:
			case SDLK_RETURN:
			case SDLK_ESCAPE:
			case SDLK_SPACE:
				return new EndTargetAction(m_game, m_world);
			}
		}
		return nullptr;
	}
